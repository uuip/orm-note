...
def log_fake_column_value(func):
    @wraps(func)
    def wrapper(field: Column):
        res = func(field)
        print(f"字段名称: {field.name}, 字段类型: {field.type}, 返回结果: {res}")
        return res

    return wrapper


@log_fake_column_value
def fake_column_value(field: Column):
    ...
    if isinstance(field.type, (VARCHAR, Text, String)):
        ...
        if "time" in field.name:
            ...
            return t.isoformat()
        return generators.cn_words(length=field.type.length or 5)
...
def generate_related_field_data(s, model, field_name, field: _RelationshipDeclared, size):
    ...
    if target_model is None:
        ...
        if len(objs) < related_size:
            _populate_model(s, target_model, size=related_size - len(objs))
...
def populate(session_maker: sessionmaker, model: DeclarativeAttributeIntercept, size=1, **kwargs):
    with session_maker() as s:
        return _populate_model(s, model, size, **kwargs)


def _populate_model(s, model, size=1, **kwargs):
    # 填充字段数据
    df = pd.DataFrame()
    for field_name, field in required_fields(model):
        if field_name in kwargs:
            df[field_name] = [kwargs[field_name]] * size
            continue
        ...
        if isinstance(field, _RelationshipDeclared):
            ...
            df[field_name] = [fake_column_value(field) for _ in range(size)]
    if size == 1:
