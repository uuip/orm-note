...
from pw.reflect import models

logger = logging.getLogger("peewee")
logger.addHandler(logging.StreamHandler())
logger.setLevel(logging.INFO)

detect, ship = models["dungeon_detect"], models["dungeon_ship"]
logcache = models["ground_logcache"]
network = models["geoip_network"]
...
network.select().where(Expression(network.GeoIp, ">>=", "1.1.4.1")).first()

ship.select(fn.sum(ship.total_reward)).scalar()
ship.select(ship, (ship.token_id * 10).alias("new_tokenid"), SQL("token_id*10").alias("abc")).first()
